dw5 = {}
dw5.n = 'Toxic scorpion'
dw5.m = 'A Toxic scorpion approaches you'
dw5.m2 = ''
dw5.o = {'f:fight','r:run'}
dw5.f = dw5.o
dw5.i = image.getImage('Toxic_scorpion')
dw5.inDialoge = false
dw5.option = false
dw5.Hp = 75
dw5.Atk = 350
...
function dw5.keypressed(key)
	if key == 'f' then
		if Atk >= 350 or math.random(1,20) == 5 then
			...
			raiseRep(120)
			raiseCash(120)
			Alert.new('Fought The Toxic scorpion','stat')
			Exp.add(495)
			gamestate = 'alert'
		else
			Hurt(320)
			Alert.new('Attacked by a Toxic scorpion','stat')
			gamestate = 'alert'
		end
	elseif key == 'r' then
		if Def >= 50 or math.random(1,20) == 5 then
			...
			Hurt(320)
			Alert.new('Attacked by a Toxic scorpion\n\nWhile Running Away','stat')
			gamestate = 'alert'
		end
	end
end
